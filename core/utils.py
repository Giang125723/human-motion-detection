...
# Lấy thông tin các bounding box và confidence đối tượng người dự đoán
def get_list_pred_infos(image, bboxes, classes=read_class_names(cfg.YOLO.CLASSES)):
    bb_list = []
    conf_list = []
    ...
    return bb_list, conf_list


# Vẽ bounding box
def draw_bboxes(image, bbox_list, conf_list):
    image_h, image_w, _ = image.shape
    ...
    for bbox, conf in zip(bbox_list, conf_list):
        ...
        cv2.putText(image, bbox_mess, (c1[0], int(c1[1] - 2)), 0, fontScale, (0, 0, 0), thickness=bbox_thick // 2,
...
def draw_bbox(image, bboxes, info=False, show_label=True, classes=read_class_names(cfg.YOLO.CLASSES)):
    ...
    for i in range(num_boxes[0]):
        ...
        if show_label:
            ...
            t_size = cv2.getTextSize(bbox_mess, 0, fontScale, thickness=bbox_thick // 2)[0]
            c3 = (c1[0] + t_size[0], c1[1] - t_size[1] - 3)
            cv2.rectangle(image, (xmin, ymin), (int(c3[0]), int(c3[1])), bbox_color, -1)  # filled
